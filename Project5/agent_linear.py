...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    Returns:
        None
    """
    current_q = (theta @ current_state_vector)[tuple2index(action_index, object_index)]
    next_q = (theta @ next_state_vector)
    if not terminal:
        theta[tuple2index(action_index, object_index)] += ALPHA*(reward + GAMMA*np.max(next_q) - current_q)*current_state_vector
    else:
        theta[tuple2index(action_index, object_index)] += ALPHA*(reward - current_q)*current_state_vector
    return None
# pragma: coderesponse end
...
if __name__ == '__main__':
    ...
    print(np.mean(np.mean(epoch_rewards_test, axis=0)[int(epoch_rewards_test.shape[1] / 2):]))
    plt.show()
