...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if epsilon < np.random.rand():
        best_action = np.unravel_index(np.argmax(q_func[state_1,state_2,:,:]), q_func.shape)
        action_index, object_index = best_action[2], best_action[3]
    else:
...
if __name__ == '__main__':
    ...
    #np.mean(epoch_rewards_test, axis=0)
